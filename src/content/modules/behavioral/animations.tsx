...
const gridFrames: GridFrame[] = [
  {
    ...
    detail: 'Reduce each cell to two or three keywords. You want to recall a story, not read a script.',
  },
  ...PROJECTS.map((project, i) => ({
    ...
    detail: 'One project rarely fills every cell, and it does not need to. Coverage across the grid is what matters.',
  })),
  {
    ...
    detail: 'A gap is not a reason to invent something. It is a signal to think harder about a real situation you have downplayed.',
  },
  {
    rows: PROJECTS.length,
    project: null,
    ...
    detail: 'Choose projects with genuinely hard components where you played a central role - ideally on the hard part.',
  },
];
...
export const storyGrid: AnimationSpec = fromFrames(
  {
    ...
    blurb: 'Projects down, themes across. Build it once and every behavioural question becomes a lookup.',
  },
  gridFrames,
  GridFrame,
);
...
const sarFrames: SarFrame[] = [
  ...
  },
  {
    ...
    detail: 'Nugget first also keeps you focused: if you cannot say it in a sentence, the story is not ready.',
  },
  {
    split: GOOD,
    focus: 1,
    ...
    detail: 'Translate rather than explain. State the impact instead of the internals, and offer to go deeper.',
  },
  {
    split: GOOD,
    focus: 2,
    ...
    detail: 'Say "I" for what you did and "we" for what the team did. Assume every question is about your role.',
  },
  {
    split: GOOD,
    focus: 3,
...
function SarFrame({ index }: { index: number }) {
  ...
  return (
    <Stage>
      ...
      <Readout
        items={[
          { key: 's', label: 'shape', value: frame.split === TYPICAL ? 'the common answer' : 'nugget + S.A.R.' },
          { key: 'f', label: 'in focus', value: frame.focus === null ? 'whole answer' : PARTS[frame.focus] },
        ]}
      />
    </Stage>
  );
...
export const sarShape: AnimationSpec = fromFrames(
  {
    ...
    blurb: 'Nugget, then situation, action, result - with most of the time on what you actually did.',
  },
  sarFrames,
  SarFrame,
);
